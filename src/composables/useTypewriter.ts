...
export function useTypewriter(text:string){
  ...
  const colors = ['#1F7A8C','#5E7CE2','#00AF54', '#007CBE', '#ddd','#957fef', '#ee4266', '#119da4','#ff4a1c', '#6f2dbd', '#f3722c', '#e63946', '#0077b6', '#8ac926', '#faa916', '#3a7ca5']
  ...
  const spaceSplittedText = computed(() => {
    ...
    const data = words.map((word, i) => {
      return {
        value: word == last || text.endsWith(word + ' ()') ? word.trim() : word + ' ', 
